...
function AppViewModel() {
  ...
  this.addIngFromInput = (ingredient, event) => {
    ...
    if (this.drinkPrices != 0) {
      ...
      if (!match) {
        this.drinkPrices.push(
          new drinkPrice(ingredient.name, (value * ingredient.price) / 10)
        );
      }
    } else {
      this.drinkPrices.push(
  ...
  this.addIngredient = (ingredient, event) => {
    ...
    if (this.drinkPrices != 0) {
      ...
      if (!match) {
        this.drinkPrices.push(
          new drinkPrice(ingredient.name, ingredient.price)
        );
      }
    } else {
  ...
  fs.readFile(ingdFile, (err, data) => {
    ...
    storedData.forEach((element) => {
      viewModel.ingredients.push(
        new ingredient(element.pos, element.name, element.price, element.level)
      );
    });
  });
